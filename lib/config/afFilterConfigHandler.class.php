<?php
/**
 * afFilterConfigHandler allows you to register filters with the system.
 *
 * @package    appFlowerStudioPlugin
 * @subpackage config
 * @author     Radu Topala <radu@appflower.com>
 */
class afFilterConfigHandler extends sfYamlConfigHandler
{
  /**
...
EOF;
  ...
  /**
   * @see sfConfigHandler
   * @abstract if there is not filter set for a key in the main app, then set the value from /plugins/appFlowerStudioPlugin/config/filters.yml
   */
  static public function getConfiguration(array $configFiles)
  {
  	$afStudioConfigFile = self::parseYaml(sfConfig::get('sf_root_dir').'/plugins/appFlowerStudioPlugin/config/filters.yml');
    ...
    foreach (array_slice($configFiles, 1) as $i => $configFile)
    {
      ...
      foreach (array_keys($previous) as $key)
      {
        if (!isset($config[$key]))
        {
          ...
          $config[$key] = $afStudioConfigFile[$key];          
        }
      }
    }
