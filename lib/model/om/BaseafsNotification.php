...
/**
 * Base class that represents a row from the 'afs_notification' table.
 *
 * 
 *
 * @package    propel.generator.plugins.appFlowerStudioPlugin.lib.model.om
 */
abstract class BaseafsNotification extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'afsNotificationPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        afsNotificationPeer
	 */
	...
	public function setMessage($v)
	{
		...
		if ($this->message !== $v) {
			$this->message = $v;
			$this->modifiedColumns[] = afsNotificationPeer::MESSAGE;
		}
	...
	public function setIp($v)
	{
		...
		if ($this->ip !== $v) {
			$this->ip = $v;
			$this->modifiedColumns[] = afsNotificationPeer::IP;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ($this->created_at !== null || $dt !== null) {
			...
			if ($currentDateAsString !== $newDateAsString) {
				...
				$this->modifiedColumns[] = afsNotificationPeer::CREATED_AT;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 6; // 6 = afsNotificationPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating afsNotification object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = afsNotificationPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = afsNotificationQuery::create()
				->filterByPrimaryKey($this->getPrimaryKey());
			$ret = $this->preDelete($con);
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseafsNotification:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseafsNotification:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseafsNotification:save:pre') as $callable)
			{
			...
			if ($isInsert) {
				...
				if (!$this->isColumnModified(afsNotificationPeer::CREATED_AT))
				{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseafsNotification:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				afsNotificationPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(afsNotificationPeer::MESSAGE)) {
			$modifiedColumns[':p' . $index++]  = '`MESSAGE`';
		}
		if ($this->isColumnModified(afsNotificationPeer::MESSAGE_TYPE)) {
			$modifiedColumns[':p' . $index++]  = '`MESSAGE_TYPE`';
		}
		if ($this->isColumnModified(afsNotificationPeer::USER)) {
			$modifiedColumns[':p' . $index++]  = '`USER`';
		}
		if ($this->isColumnModified(afsNotificationPeer::IP)) {
			$modifiedColumns[':p' . $index++]  = '`IP`';
		}
		if ($this->isColumnModified(afsNotificationPeer::CREATED_AT)) {
			$modifiedColumns[':p' . $index++]  = '`CREATED_AT`';
		}
		if ($this->isColumnModified(afsNotificationPeer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}

		$sql = sprintf(
			'INSERT INTO `afs_notification` (%s) VALUES (%s)',
	...
	{
		$pos = afsNotificationPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['afsNotification'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['afsNotification'][$this->getPrimaryKey()] = true;
	...
	{
		$criteria = new Criteria(afsNotificationPeer::DATABASE_NAME);
		$criteria->add(afsNotificationPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of afsNotification (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     afsNotification Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     afsNotificationPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new afsNotificationPeer();
		}
		return self::$peer;
	}
	...
	{
		return (string) $this->exportTo(afsNotificationPeer::DEFAULT_STRING_FORMAT);
	}
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseafsNotification:' . $name))
		{
